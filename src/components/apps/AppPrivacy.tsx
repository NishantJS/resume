...
/* The generated policy's sections, for apps that do not ship one of
   their own. Ids double as the contents rail — keep in sync with the
   <section>s in GeneratedPolicy. */
...
/** Boilerplate policy, assembled from the app's data table. Generic by
    design: it describes an app with an account and a server. */
const GeneratedPolicy: FC<{
  app: AppMeta;
  privacy: NonNullable<AppMeta["privacy"]>;
  inks: Inks;
}> = ({ app, privacy, inks }) => {
  const { ink, inkDim, border } = inks;
  const { support, name } = app;
  return (
    <>
...
/** A policy the app ships with, reproduced as written. Blocks carry
    inline markup rather than JSX so the document stays in the data
    file, where it can be diffed against the copy Play was given. */
const VerbatimPolicy: FC<{ doc: PolicyDoc; inks: Inks }> = ({ doc, inks }) => {
  const { ink, inkDim, border } = inks;

  const renderBlock = (b: PolicyBlock, i: number) => {
    if (b.kind === "p") return <p key={i}><RichText text={b.text} ink={ink} /></p>;

    if (b.kind === "ul") {
      return (
        <ul key={i}>
          {b.items.map(item => <li key={item}><RichText text={item} ink={ink} /></li>)}
        </ul>
      );
    }
    ...
    if (b.kind === "callout") {
      return (
        <aside key={i} className="app-callout" style={{ borderColor: border, background: inks.panel }}>
          <p className="app-callout-title" style={{ color: ink }}>
            <RichText text={b.title} ink={ink} />
          </p>
          {b.text.map(t => <p key={t}><RichText text={t} ink={ink} /></p>)}
        </aside>
      );
    }

    /* Two columns get the narrow pair table; three or more get the wider
       one. Either way it scrolls inside its own box rather than pushing
       the page sideways on a phone. */
    return (
      <div key={i} className="mt-5 overflow-x-auto rounded-xl border" style={{ borderColor: border }}>
        <table
          className={`app-table${b.head.length === 2 ? " app-table--pairs" : ""}`}
          style={{ borderColor: border }}
        >
          <colgroup>{b.head.map(h => <col key={h} />)}</colgroup>
          <thead>
            <tr>{b.head.map(h => <th key={h} scope="col" style={{ color: inkDim }}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {b.rows.map(row => (
              <tr key={row[0]}>
                <th scope="row"><RichText text={row[0]} ink={ink} /></th>
                {row.slice(1).map((cell, c) => (
                  <td key={c}><RichText text={cell} ink={ink} /></td>
                ))}
              </tr>
            ))}
          </tbody>
  ...
  return (
    <>
      <section className="app-reveal">{doc.intro.map(renderBlock)}</section>

      {doc.sections.map(sec => (
        <section key={sec.id} id={sec.id} className="app-reveal">
          <h2 style={{ color: ink }}>{sec.heading}</h2>
          {sec.blocks.map(renderBlock)}
        </section>
      ))}

      <p className="app-reveal mono text-xs mt-10" style={{ color: inkDim }}>{doc.footer}</p>
    </>
  );
};

/** Privacy policy — the URL that goes in the Play Console listing. */
const AppPrivacy = () => {
  ...
  /* One of the two always exists: an app either ships its own policy or
     supplies the table the generic one is assembled from. */
  const sections = doc ? doc.sections.map(s => ({ id: s.id, label: s.label })) : GENERIC_SECTIONS;
  const lead = doc ? doc.lede : app.privacy?.stance ?? "";
  const stamp = doc
    ? `Last updated ${formatDate(doc.updated)} · ${doc.applies}`
    : `Effective ${formatDate(app.privacy?.effective ?? "")} · Version ${app.release.version}`;
        ...
        <AppSection id="policy" kicker="The policy" inks={inks}>
          <div className="grid gap-12 lg:grid-cols-[190px_1fr] lg:gap-16">
            ...
            <div className="app-prose min-w-0 max-w-3xl" style={{ color: inkLow, borderColor: border }}>
              {doc
                ? <VerbatimPolicy doc={doc} inks={inks} />
                : app.privacy && <GeneratedPolicy app={app} privacy={app.privacy} inks={inks} />}
            </div>
          </div>
        </AppSection>
      </div>
